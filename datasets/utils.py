...
class preprocessor(Dataset):
    ...
    def __getsingleitem__(self, index):
        ...
        # radiomap, sampled_map, physics_rm \in
        # (0, 1) -> (-1, 1)

        # NOTE DEBUG
        # radiomap = radiomap.astype(np.float32) * 2.0 / 255.0 - 1.0
        radiomap = radiomap.astype(np.float32) / 255.0
        ...
        if self.cfg.MODEL.SINGLE_HEIGHT:
            ...
            sampled_map = np.full_like(radiomap, 0.0, dtype=np.float32)
...
class BaseDataset(object):
    ...
    def sample_all_heights_coordinate(self, radiomap, building, num_sample, separately=True):
        ...
        sampled_map = np.zeros(shape=radiomap.shape, dtype=np.float32)
