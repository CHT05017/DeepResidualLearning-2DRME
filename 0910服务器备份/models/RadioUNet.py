...
Levie et al., 2021 IEEE TWC

Source code could be found at 
https://github.com/RonLevie/RadioUNet/blob/master/lib/modules.py

## Training Methods:
- `phase = "firstU"` primarily, then shift to `phase = "secondU"`
    to achieve a two-stage training. For evaluation, must use output2, 
    and during inference, phase must be set to something other than "firstU" 
    (e.g., phase="secondU"). `output2` is the final feature used for evaluation
"""
...
class RadioUNet(nn.Module):
    """
    ### Args:
    - `inputs`: channel number of input images
    - `phase`: Training phase. "firstU" is expected primarily, then 
        shift to "secondU"    
    """
...
if __name__ == "__main__":
    ...
    plt.figure()
    plt.imsave("./test2.jpg", output[-1][0, 0].detach().cpu().numpy(),
                 cmap="viridis")
    print(output[-1].shape)

# python -m models.RadioUNet
